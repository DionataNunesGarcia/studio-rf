...
class SpecialistsCategoriesDatatablesService extends DatatablesService
{
    /**
     * @return array[]
     */
    protected array $_customFields = [
        "id" => [
            'field' => 'SpecialistsCategories.id',
...
//            }
    ...
    private function handleResponse(array $results) :array
    {
        ...
        foreach ($results as $item) {
            $specialists = count($item->specialists);
            $response[] = [
                'id' => $item->id,
                'name' => $item->name,
                'blogs' => $specialists,
                ...
                'actions' => [
                    ...
                    'delete' => $specialists ? false : $this->hasPermission('delete', $this->getModel(), $item->id),
                ],
            ];
        }
        return $response;
